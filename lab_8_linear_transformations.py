# -*- coding: utf-8 -*-
"""Lab 8 - Linear_Transformations.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16U-rgFq2LgYN4MkWaD4P9klUVIJbripQ
...
def reflect(image,a,b): 
  a2 = np.power(a,2)
  b2 = np.power(b,2)
  A = (1/(a2+b2))*np.array([[a2- b2, 2*a*b], 
                            [2*a*b, b2-a2]])
  B = np.array(image)
  C = np.matmul(A,B)
  return C
...
comp_matrix= np.array([[ 0.1903749 , -0.9246781 ],
                       [-0.9246781 , -0.76149961]])# Put the transformation matrix which results for the sequence of transformations here.
...
def area(A):
  v = A[0]
  a = np.power(np.dot(v,v),1/2) #length of v
  w = A[1]
  b = np.power(np.dot(w,w),1/2)
  cos = np.dot(v,w)/(a*b)
  theta = np.arccos(cos)
